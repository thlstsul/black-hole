...
use blackhole_shared::{EngineCommand, InputContext, KeyEvent, KeyState, Modifiers, SchemeId};
...
fn main() {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "-h" | "--help" => {
                println!("Blackhole IME CLI Test Harness");
                println!("Usage: blackhole-cli [OPTIONS]");
                println!();
                println!("Options:");
    ...
    tracing::info!("Blackhole IME CLI Test Harness");
    ...
    loop {
        ...
        for ch in line.chars() {
            ...
            match result {
                blackhole_shared::SchemeResult::Composing {
                    code,
                    candidates,
                    selected_index,
                    ..
                ...
                blackhole_shared::SchemeResult::Committed { text } => {
                    tracing::info!("  committed: {}", text);
                }
                blackhole_shared::SchemeResult::Ignored => {
                    // 忽略非字母输入
                }
            }
        }
        ...
        if let blackhole_shared::SchemeResult::Committed { text } = result {
            tracing::info!("  committed: {}", text);
        }
